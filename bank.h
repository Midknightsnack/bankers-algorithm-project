...
public:
  ...
  bool is_safe(int id, const ext_vector<int>& req) { // TODO: determine if alloc is safe
    bool answer = false;
    ext_vector<int> temp_bank = this->avail;
    temp_bank -= req;

    for(size_t i = 0; i < this->avail.size(); ++i){   //go through customer list
      if(i == id) continue;
      if(temp_bank >= this->customers[i]->create_req()){  //temporarily grant request to customer
        answer = true;
      }
    }
    return answer; 
